...
class Visualizer(object):
    ...
    def show_maze(self, display_mode = True):
        ...
        if self.media_filename:
            output_folder = "./temp"
            os.makedirs(output_folder, exist_ok=True)  # Create the "temp" folder if it doesn't exist

            output_path = os.path.join(output_folder, self.media_filename)  # Construct the output path
            fig.savefig("{}.png".format(output_path), frameon=None)
    ...
    def show_maze_solution(self, episodes = None, display_key = False, learning_rate = None, discount_factor = None):
        ...
        for i in range(1, self.maze.solution_path.__len__()):
            if self.maze.solution_path[i][0] not in list_of_backtrackers and\
                ...
                # Add the title to the subplot with step information
                if episodes is not None:
                    if learning_rate is not None:
                        self.ax.set_title(f"Episode {episodes+1} & Step {i}\n Learning Rate : {learning_rate}\n Discount_factor : {discount_factor}")
                else:
        ...
        if self.media_filename:
            ...
            fig.savefig(os.path.join(output_folder, "{}{}.png".format(self.media_filename, "_solution")), frameon=None)
        plt.close()
    ...
    def animate_maze_solution(self):
        ...
        self.ax = ax  # Change self.ax to ax here
        ...
        anim = animation.FuncAnimation(fig, animate, frames=self.maze.solution_path.__len__(),
                                    interval=4, blit=True, repeat=False)
        ...
        if self.media_filename:
            ...
            mpeg_writer = animation.FFMpegWriter(fps=24, bitrate=1000,
                                                codec="libx264", extra_args=["-pix_fmt", "yuv420p", "-preset", "slow"])
            anim = animation.FuncAnimation(fig, animate, frames=range(len(self.maze.solution_path)), interval=100)
            anim.save(os.path.join(output_folder, "{}{}{}x{}.mp4".format(self.media_filename, "_solution_", self.maze.num_rows, self.maze.num_cols)),
                    writer=mpeg_writer)
