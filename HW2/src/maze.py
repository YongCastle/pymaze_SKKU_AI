...
class Maze(object):
    ...
    def validate_neighbours_solve(self, neighbour_indices, k, l, k_end, l_end, method = "fancy"):
        ...
        if method == "fancy":
            ...
            neigh_list = [n for n in neighbour_indices if not self.grid[k][l].is_walls_between(self.grid[n[0]][n[1]])]
    ...
    def is_wall(self, next_state, current_state):

        #If Agent go Out side Maze
        if(next_state == current_state):
            return True
    
        next_k, next_i = next_state
        ...
        # print(current_state)
        # print("action: "+ action)
        # print("is wall: " + str(self.grid[curr_k][curr_i].walls))
        # print(self.grid[curr_k][curr_i].walls[action])
        # input("")

        return self.grid[curr_k][curr_i].walls[action]
